...
class CLCRadius:
    ...
    def _change_crs(
        ...
        if not os.path.exists(path_raster):
            ...
            gdal.Warp(
                os.path.join(path_in, "tempo_4326.tif"),
                input,
            ...
            topts = gdal.TranslateOptions(
                ...
                bandList=list(range(1, 2)),
            )  # gdal.GDT_Byte

            gdal.Translate(
                os.path.join(path_in, "U2018_CLC2018_V2020_20u1_4326.tif"),
                os.path.join(path_in, "tempo_4326.tif"),
                options=topts,
            )
    ...
    def _radius_around_points(lon, lat, radius=500, save=True):
        ...
        df["circus_" + str(radius)] = df.geometry.apply(
            lambda x: utils.create_polygon_radius(x, radius)
        )

        df = gpd.GeoDataFrame(
            df[["X_COORD", "Y_COORD", "circus_" + str(radius)]],
            geometry=df["circus_" + str(radius)],
            crs="EPSG:4326",
        )
    ...
    def _mask_corine_around_point(
        ...
        if str(meta["crs"]) == "EPSG:3035":
            raise ValueError(
                "You must convert CLC into EPSG:4326 using preprocess_corine()"
            )
        ...
        x = np.ma.array(clc, dtype=np.int16, mask=(mask).astype(bool), fill_value=0)
        results = x.filled()
        del clc
        ...
        gdal.Translate(
            os.path.join(path_in, "tempo" + "_" + str(self.radius) + ".tif"),
            os.path.join(path_in, "tempo.tif"),
            options=topts,
        )
    ...
    def _get_intersection_area(self, output):
        ...
        df = gpd.read_file(
            os.path.join(
                "./radius_" + str(self.radius), "./radius_" + str(self.radius) + ".shp"
            )
        )
    ...
    def vectorize_corine_around_point(
        self,
        lon,
        lat,
        path_corine_wgs84=r"./CLC_2018/u2018_raster100m/U2018_CLC2018_V2020_20u1_4326_crop.tif",
        ...
        if os.path.exists(os.path.join("./tempo_shp", "tempo_shp.shp")):
            driver.DeleteDataSource(
                os.path.join(os.path.join("./tempo_shp", "tempo_shp.shp"))
            )

        outDatasource = driver.CreateDataSource(
            os.path.join("./tempo_shp", "tempo_shp.shp")
        )
